...
| 2026-08-06 | Rename `ucgBackend/sonaged_web/mobileFlutter` → `backend-api/frontend/mobile` en Git | — | ✅ déjà fait (commits `5e9fad2`, `54607ad`) | La mention « jamais rejoué en `git mv` » plus haut dans ce document est caduque |
| 2026-08-06 | Push de la branche + premier run CI GitHub | — | 🔴 **bloqué** | `git push` échoue : `fatal: repository 'https://github.com/keane-kane/master-ucg.git/' not found` — accès/existence du dépôt distant à vérifier côté GitHub avant de rejouer ; 85 commits en attente sur `origin/main` |
| 2026-08-07 | ADR-0012 achevé : `Depotoir` et `Alert` migrés en UUID v7 — dernières entités `waste` encore en `Long` | 38 fichiers référençant `depotoirId` + 6 référençant `alertId` (entités, DTO, repositories, services, contrôleurs, ports `WasteReadModel`/`IngestionMetrics`/`CollectionPerformance`, événements `MeasurementRecorded`/`AlertRaisedEvent`/`SensorSilenceDetected`/`SensorBackOnline`) ; changelog `2.22.0` ; ~20 fichiers de tests corrigés (littéraux `Long` → UUID stables dérivés d'un entier, cf. helper `uuid(n)` répété par fichier) ; `frontend/.../api-endpoints.ts` (`idType`) + 4 composants (`device`, `reports`, `collection-route`, plus les deux déjà couverts) | ✅ **compilé et testé** — voir entrée suivante | `depotoir`/`sensor`/`measurement`/`collectionpassage`/`alert` purgés par `2.22.0` (voir commentaire du changelog) : capteurs à ré-enrôler, historique non reconstituable sur une base non jetable |
| 2026-08-07 | Build réel effectué : JDK Corretto 22 (`~/.jdks/corretto-22.0.2`) trouvé sur la machine, `./mvnw test` exécuté pour la première fois de cette session de travail | — | ✅ **BUILD SUCCESS, 266 tests, 0 échec** (1 ignoré, `SonagedApplicationTests`, comme toujours) | Deux bugs réels trouvés et corrigés que la compilation seule ne pouvait pas voir : import `UUID` manquant dans `AlertRaisedEvent.java` (erreur de compilation) ; `AdministrationAuthorizationTest` postait sur `.../stops/1/collected` — `"1"` n'est plus un `UUID` valide, Spring renvoyait 500 avant même le contrôle d'autorisation testé (corrigé en UUID littéral). `ApplicationContextLoadsTest`/`LiquibaseSchemaMatchesEntitiesTest`/`JpaMappingBootstrapTest` ont tous les trois réellement tourné (pas de sommeil faute de Postgres) et validé les changelogs `2.18.0`→`2.22.0` contre les entités |
| 2026-08-07 | G3 (suite) — campagnes de sensibilisation et quiz, au-dessus de l'`awarenessmessage` existant | `AwarenessCampaign`, `Quiz`, `QuizQuestion`, `QuizResponse` (entités) + repositories + `AwarenessCampaignService(Impl)` + `QuizService(Impl)` + `AwarenessCampaignController` (`/v1/awareness/campaigns`) + `QuizController` (`/v1/quizzes`) ; `AwarenessService.schedule(...)` étendu (méthode `default` rétrocompatible, `campaignId` optionnel) ; `SecurityConfiguration` : exemption d'écriture citoyenne pour `POST /v1/quizzes/*/answers` ; changelog `2.23.0` ; tests `AwarenessCampaignServiceImplTest`, `QuizServiceImplTest`, + 2 cas ajoutés à `AdministrationAuthorizationTest` | ✅ **BUILD SUCCESS, 266 tests, 0 échec** | Une seule tentative de quiz par citoyen (contrainte applicative + `uk_quizresponse_quiz_user`) ; la bonne réponse n'est jamais exposée avant correction (`QuizController` la projette hors des DTO de lecture) ; pas de frontend — demande non exprimée pour ce lot |
| 2026-08-06 | Rôles `SUPERVISEUR`/`TECHNICIEN_IOT` semés (Tome 1 §2) ; `SecurityConfiguration` ouverte en conséquence | changelog `2.21.0` ; `SecurityConfiguration.java` (`/v1/supervision/**` séparé du bloc administration, nouveau matcher `POST`/`DELETE /v1/devices/**`) ; 2 tests ajoutés à `AdministrationAuthorizationTest` | 🟡 code écrit, **non compilé** | Sans cette édition les deux rôles étaient inertes : `hasAnyRole(ADMIN,SUPER_ADMIN)` au niveau filtre bloquait avant tout `@PreAuthorize`/permission. `CITOYEN` et `CONTROLEUR_BALAYAGE` volontairement PAS ajoutés (voir commentaire du changelog) |
| 2026-07-11 | **P0-1** Hacher le mot de passe fourni à l'inscription | `backend-api/.../service/impl/AuthServiceImpl.java` | ✅ implémenté (non compilé) | Compilation ; l'inscription hache bien le mot de passe saisi ; réinitialiser les comptes créés avec l'ancien `Sonaged@123` |
| 2026-07-11 | **P0-2** Externaliser les secrets (props env) + `.gitignore` racine | `application.properties`, `.gitignore` (racine), `backend-api/.env.example` | ✅ partiel (non compilé) | Définir les variables d'env (`DB_PASSWORD`…) avant démarrage ; **retirer du suivi Git + roter** `google-services.json` et les secrets déjà dans l'historique ; externalisation du secret JWT reportée à P0-A (Keycloak) |
...
| 2026-07-25 | **P1-1** Nettoyer les dépendances backend (springfox + 2e lib JWT) | `backend-api/pom.xml` | ✅ implémenté (non compilé) | `./mvnw dependency:tree` pour confirmer l'absence de springfox / com.auth0 dans le classpath ; démarrage app + Swagger UI (`/swagger-ui`) toujours servi par springdoc ; login JWT toujours OK (jjwt conservé) |
...
`mobileFlutter/` → `mobile/`), mais **jamais rejoué en `git mv`** : Git ne voit encore que des
suppressions sous l'ancien nom et un répertoire non suivi sous le nouveau. Ce journal, `CLAUDE.md`,
`README.md`, `ROADMAP.md` et les autres pages de `docs/` référencent désormais `backend-api/` ;
l'historique Git, lui, reste à réconcilier (`git add` avec détection de renommage prêt, non commité).

`docs/KNOWLEDGE_MAP.md` §7.2 listait « Dockerfile applicatif » ❌ (seule la CI existait,
`.github/workflows/backend.yml`, elle-même déjà verte en local). Le répertoire renommé contenait en
fait le travail déjà fait, non documenté :

- **`Dockerfile`** multi-étage (build Maven → JRE Alpine), utilisateur non-root, `MaxRAMPercentage`
  pour que la JVM lise la limite du cgroup plutôt que celle de l'hôte, arrêt propre en forme exec
  (PID 1 reçoit SIGTERM directement).
- **`.dockerignore`** — exclut `target/`, secrets locaux (`.env`, `application-local.*`), et tout ce
  qui n'a pas à survivre dans l'image.
- **Actuator ajouté au classpath** (`pom.xml`) : la clé `management.endpoints.web.exposure.include`
  existait dans `application.yml` depuis P2-4 (2026-07-26) mais restait **inerte**, faute de
  dépendance. Seul `/actuator/health` est ouvert dans `SecurityConfiguration` (jamais `/actuator/**`
  — les autres points publient beans/config/variables d'environnement, soit l'inventaire des
  secrets). `show-details: never` réduit la réponse à UP/DOWN. Le contributeur de santé `mail` est
  désactivé : un SMTP indisponible ne doit pas déclarer l'API DOWN et la faire redémarrer en boucle,
  alors qu'elle sert toujours alertes/tournées/cartes sans lui.

**Non vérifié** : `docker build` n'a pas été exécuté ici (pas de démon Docker dans cet
environnement), ni `mvn clean package` sur le répertoire renommé.

| Date | Tâche | Fichiers | Statut | À vérifier manuellement |
|---|---|---|---|---|
| 2026-08-06 | **P2-2** Dockerfile applicatif + sonde Actuator ; réconciliation Git du renommage `ucgBackend`→`backend-api` (docs) | `backend-api/Dockerfile`, `.dockerignore`, `pom.xml`, `application.yml`, `SecurityConfiguration.java`, `.github/workflows/backend.yml` | 🟡 code présent, non compilé/buildé ici | `docker build .` depuis `backend-api/` ; `./mvnw clean package` après le renommage ; `git add -A` (détection de renommage) puis commit pour que Git suive enfin `backend-api/` |
...
- **Impacts / non traité** :
  - **Chemin par défaut relatif** (`file:../datas`) : valide seulement si le backend est lancé depuis `backend-api/`. Pour un jar déployé, régler `GEOJSON_DIR` (ou pointer un `classpath:` après avoir empaqueté les fichiers).
