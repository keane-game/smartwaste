...
- **1er module migré — `supervision`** (choisi parce que **rien ne dépend de lui** : migration sans effet de bord sur les autres). 7 fichiers déplacés vers `sonaged.ucg.supervision` : `DashboardController/Service/ServiceImpl`, `DepartmentState`, `SupervisionStats(+Controller/Service)`. Organisation **par domaine et non par couche** (contrôleur, service et DTO dans le même package), conformément à l'intention modulaire.
- **Résultat** : `UcgModularityTests` **n'est plus vide** — le module `supervision` déclare de vrais beans et `verify()` passe. Build complet vert.
...
1. **Rompre le cycle `identiteacces` ↔ `communication`** — couplage bidirectionnel réel : `ValidationServiceImpl` (identité) appelle `NotificationService` (communication), et `NotificationServiceImpl` importe `UserEntity`/`Validation` (identité). Solution conforme à ADR-0010 §2 : l'identité **publie un événement** (`ActivationCodeIssued`) portant les données nécessaires ; la communication l'écoute et cesse d'importer les entités d'identité. ⚠️ Modifie le comportement d'envoi des e-mails — **à faire avec des tests**.
2. Migrer `communication`, puis `identiteacces`.
