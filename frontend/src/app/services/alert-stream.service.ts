 ...
 * paramètre d'URL (le jeton finit alors dans les journaux d'accès et l'historique) ou d'ouvrir
 ...
 * back-off simple.
 *
 * <p><b>Ce que le flux porte réellement aujourd'hui.</b> Le backend émet trois types
 * d'événements : `connected` (accusé d'ouverture), `alert` (charge utile : l'alerte) et
 * `heartbeat` (maintien de connexion). Seul `AlertRaisedEvent` est diffusé —
 * `MeasurementRecorded`, `SensorSilenceDetected` et `SensorBackOnline` existent comme événements
 * de domaine mais ne sont pas rediffusés sur le flux.
 */
...
export class AlertStreamService implements OnDestroy {
  ...
  private async openStream(): Promise<void> {
    ...
    // Le JWT est rangé sous `currentUser.bearer` (cf. AuthService.login / getAuthToken), pas
    // sous `token` ni `access_token` : sans cette lecture, le flux `/v1/**` répond 401 et reste
    // muet, sans erreur visible.
    const token = JSON.parse(localStorage.getItem('currentUser') || '{}').bearer;
  ...
  private handleFrame(frame: string): void {
    ...
    // `fetch` s'exécute hors de la zone Angular : sans `zone.run`, la vue ne serait pas
    // rafraîchie à l'arrivée d'une alerte.
