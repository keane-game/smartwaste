...
class MapsReceiver extends StatefulWidget {
  ...
  const MapsReceiver({super.key, required this.deviceid});
...
class MapsReceiverState extends State<MapsReceiver> {
  ...
  void initState() {
    ...
    subscription =
      ...
      setState(() {
        currentLatitude = values['latitude'];
        currentLongitude = values['longitude'];
      });
      mapController.animateCamera(
        CameraUpdate.newCameraPosition(
          CameraPosition(
              target: LatLng(values['latitude'], values['longitude']),
              zoom: 17),
        ),
      );
      _add(
        LatLng(values['latitude'], values['longitude']),
      );
    });
  }
